...
test('list item  with a photo', () => {
	const { toJSON, queryByText, getByText } = render(
		<ListItem item={itemCore} />
	);
	expect(toJSON()).toMatchSnapshot();
	// expect(queryByText("Woodleigh - Gate 1")).not.toBeNull();
	// expect(getByText('Offline')).not.toBeNull();
	// expect(getByText('Last Seen : never')).not.toBeNull();
	// expect(getByText('AC233FC03E71')).not.toBeNull();
