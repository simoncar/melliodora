...
const itemURLlinking = {
	params: {
		...
		story: {
			...
			descriptionMyLanguage:
				"google https://google.com is a website mys https://mystamford.edu.sg/somepage is a website, test@smartcookies.io is an email and 444-555-6666 is a phone number",
...
test("show story on screen", () => {
	...
	const { toJSON, queryByTestId, getByTestId, queryByText } = render(
		<Story route={itemCore} navigation={navigation} />
	);
...
test("show story with URL in content to test linking", () => {
	...
	const { toJSON, queryByText } = render(<Story route={itemURLlinking} navigation={navigation} />);
...
test("show story without chat", () => {
	...
	const { toJSON, queryByTestId, queryByText } = render(<Story route={itemNoChat} navigation={navigation} />);
...
test("show story no calendar no image", () => {
	...
	const { toJSON, queryByTestId, queryByText } = render(
		<Story route={itemNoCalendarNoImage} navigation={navigation} />
	);
