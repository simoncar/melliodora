...
class FeatureListItem extends Component<TProps> {
	...
	deleteStory = () => {
		...
		firebase.firestore().collection(globalAny.domain).doc("feature").collection("features").doc(storyID).delete();
	};
	...
	adminMode = () => {
		if (this.props.editMode) {
			return (
				<TouchableOpacity style={styles.touchable} onPress={() => this.confirmDelete()}>
			...
			<View style={styles.newsContentLine}>
				<TouchableOpacity onPress={() => this.props.navigation.navigate("storyMore", this.props.story)}>
					<View style={styles.itemRow}>
						<Image style={styles.itemImage} source={{ uri: uri }} />
